...
namespace UnityHFSM
{
        ...
        IActionable<TEvent>
    {
        ...
        /// <param name="overwrite">Optionally overwrite existing transitions?</param>
        public void AddTransition(TransitionBase<TStateId> transition, bool overwrite = false)
        {
            InitTransition(transition);

            StateBundle bundle = GetOrCreateStateBundle(transition.from);
            if (overwrite && bundle.transitions.Count > 0)
                bundle.transitions = null;
